...
namespace DeepWoodsMod
{
    public class DeepWoodsBuilder
    {
        ...
        private TileSheet infestedOutdoorTileSheet;
        private TileSheet outdoorTileSheet;
        ...
        public DeepWoodsBuilder(DeepWoods deepWoods, DeepWoodsRandom random, DeepWoodsSpaceManager spaceManager, Map map, Dictionary<ExitDirection, Location> exitLocations)
        {
            ...
            {
                this.outdoorTileSheet = this.defaultOutdoorTileSheet;
            }
        ...
        public static void RemoveInfested(DeepWoods deepWoods, Map map)
        {
            DeepWoodsBuilder deepWoodsBuilder = new DeepWoodsBuilder(deepWoods, new DeepWoodsRandom(deepWoods, deepWoods.Seed), new DeepWoodsSpaceManager(deepWoods.mapWidth.Value, deepWoods.mapHeight.Value), map, null);
            deepWoodsBuilder.RemoveInfested();
        }

        private void RemoveInfested()
        {
            ReplaceInfestedLayerTiles(backLayer);
            ReplaceInfestedLayerTiles(buildingsLayer);
            ReplaceInfestedLayerTiles(frontLayer);
            ReplaceInfestedLayerTiles(alwaysFrontLayer);
        }

        private void ReplaceInfestedLayerTiles(Layer layer)
        {
            int xMin = layer.Tiles.Array.GetLowerBound(0);
            int xMax = layer.Tiles.Array.GetUpperBound(0) + 1;
            int yMin = layer.Tiles.Array.GetLowerBound(1);
            int yMax = layer.Tiles.Array.GetUpperBound(1) + 1;
            ...
            for (int x = xMin; x < xMax; x++)
            {
                for (int y = yMin; y < yMax; y++)
                {
                    ...
                    {
                        if (tile.TileSheet == this.infestedOutdoorTileSheet)
                        {
                            layer.Tiles[x, y] = new StaticTile(layer, this.defaultOutdoorTileSheet, BlendMode.Alpha, tile.TileIndex);
                        }
                    }
                    else if (tile is AnimatedTile animTile)
                    {
                        layer.Tiles[x, y] = new AnimatedTile(layer, ReplaceInfestedFrameTiles(animTile.TileFrames), animTile.FrameInterval);
                    }
                }
            }
        }

        private StaticTile[] ReplaceInfestedFrameTiles(StaticTile[] frameTiles)
        {
            for (int i = 0; i < frameTiles.Length; i++)
            {
                Tile tile = frameTiles[i];
                if (tile.TileSheet == this.infestedOutdoorTileSheet)
                {
                    frameTiles[i] = new StaticTile(tile.Layer, this.defaultOutdoorTileSheet, BlendMode.Alpha, tile.TileIndex);
                }
            }
            return frameTiles;
        }
                ...
                for (int y = minY; y <= maxY; y++)
                {
                    if (IsTileBrightGrass(x, y, true))
                    {
                        if (deepWoods.doesTileHaveProperty(x, y - 1, "Water", "Back") != null
                            ...
                            && this.random.CheckChance(DeepWoodsSettings.Settings.Generation.ChanceForWaterLily))
                        {
                            if (this.random.CheckChance(DeepWoodsSettings.Settings.Generation.ChanceForBlossomOnWaterLily))
                                PlaceAnimatedTile(buildingsLayer, outdoorTileSheet, WATER_LILY_WITH_BLOSSOM, x, y - 1, PlaceMode.DONT_OVERRIDE, this.random.GetRandomValue(WATER_LILY_FRAMERATES));
                            else
                                PlaceAnimatedTile(buildingsLayer, outdoorTileSheet, WATER_LILY, x, y - 1, PlaceMode.DONT_OVERRIDE, this.random.GetRandomValue(WATER_LILY_FRAMERATES));
        ...
        private void GenerateLichtung()
        {
            ...
            {
                int numAdditionalLights = ((maxX - minX) * (maxY - minY)) / Math.Max(1, DeepWoodsSettings.Settings.Generation.NumTilesPerLightSource);
        ...
        private bool PlaceTile(Layer layer, int tileIndex, int x, int y, PlaceMode placeMode = PlaceMode.DONT_OVERRIDE)
        {
            ...
            {
                layer.Tiles[x, y] = new StaticTile(layer, outdoorTileSheet, BlendMode.Alpha, tileIndex);
                return true;
            }
        ...
        private void GenerateForestPatch(xTile.Dimensions.Rectangle rectangle)
        {
            ...
            {
                int maxLightSources = Math.Max(1, numFillTiles / Math.Max(1, DeepWoodsSettings.Settings.Generation.NumTilesPerLightSource));
